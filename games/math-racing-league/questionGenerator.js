...
function bronzeQuestion(difficulty) {
...
function bronzeQuestionV2(difficulty) {
  const level = difficultyIndex(difficulty);
  ...
  if (level === 1) {
    const type = pick(["add", "subtract", "multiply", "missing", "sequence", "place", "word"]);
...
function silverQuestion(difficulty) {
  const level = difficultyIndex(difficulty);
...
function goldQuestion(difficulty) {
  const level = difficultyIndex(difficulty);
...
function championQuestion(difficulty) {
  const level = difficultyIndex(difficulty);
...
export function generateRaceQuestion(leagueId, difficulty) {
  const generator = {
